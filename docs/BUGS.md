...
### 初步原因
- 后端搜索在 `best_substring_index` 中对 UTF-8 字符串按字节递增索引。
- 当查询包含中文时，`start = idx + 1` 可能落在非 UTF-8 边界，触发 panic 导致应用退出。

### 解决方案
- 改为使用 `find` 直接取首个匹配位置，避免手动字节递增。
- 增加中文搜索的回归测试用例。
- 前端补充输入法组合态（IME）防误触逻辑，避免回车提交被当作执行。

### 状态
- 已修复：`prompt-launcher/src-tauri/src/prompts.rs`、`prompt-launcher/src/routes/+page.svelte`。
